...
def blog_summary(item: dict, related_articles: list[dict]) -> str:
    ...
    if "손승원" in title and "음주운전" in title:
        return (
            ...
            "민감한 사안이라 감정적으로 키우기보다 기사에 나온 내용 위주로 정리해보겠습니다."
        )
    if item.get("safety_flags"):
        return (
            f"{entity} 관련 이야기는 방송 장면과 온라인 주장성 내용이 같이 묶이면서 커졌습니다. "
            "제목에 센 표현이 들어가다 보니 더 크게 보이지만, 기사에 나온 내용과 주장처럼 읽히는 부분은 따로 나눠서 정리하는 게 자연스러웠습니다. "
            "특히 임신설이나 편집 요구설처럼 민감한 말은 기사 속 표현과 방송 내용이 섞여 읽히는 부분이 있었습니다. "
            "본문에서는 어디까지가 기사 내용이고 어떤 부분이 아직 애매한지 위주로 정리해봤습니다."
        )
...
def sensitive_news_summary(item: dict, sentences: list[str]) -> list[str]:
    ...
    if edit and edit != broadcast:
        paragraphs.append(
            ...
            "다만 이 부분은 기사 안에서도 주장성 내용으로 다뤄진 흐름입니다."
        )
    else:
        paragraphs.append(
            "다만 임신설이나 편집 요구처럼 민감한 표현은 기사 안에서도 주장성 내용으로 함께 다뤄졌습니다. "
            "그래서 기사에 나온 주장과 실제 방송 장면은 나눠서 정리했습니다."
        )
    return paragraphs[:3]
...
def core_summary_fallback(item: dict) -> str:
    ...
    if item.get("safety_flags"):
        return (
            ...
            "본문에서는 기사에 나온 내용과 반복되는 키워드만 분리해서 정리했습니다."
        )
    return (
...
def public_reaction_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    if item.get("safety_flags"):
        return (
            f"{entity} 관련 반응은 온라인 주장성 내용과 방송 장면이 함께 묶인 흐름입니다. "
            "본문에서는 기사에 나온 내용을 중심으로 정리했습니다."
        )
    return (
...
def closing_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    if item.get("safety_flags"):
        return (
            f"{entity} 관련 이야기는 제목만 보고 단정하기보다 조금 차분하게 정리하고 싶었습니다. "
            "새로운 내용이 나오면 이어서 다시 정리해보겠습니다!"
        )
    if "한예리" in title and ("백상" in title or "워스트" in title):
        return (
