...
def title_candidates(item: dict) -> list[str]:
    ...
    if "한예리" in title and ("백상" in title or "워스트" in title):
        ...
        candidates = [
            f"{entity} 관련 보도, 방송 장면과 온라인 주장 정리",
            f"{entity} 관련 이야기, 기사 속 흐름 정리",
        ...
        candidates = [
            ...
            f"{secondary or entity} 흐름 정리, 반응까지 쉽게 정리",
        ]
...
def blog_summary(item: dict, related_articles: list[dict]) -> str:
    ...
    if "아이오아이" in title and any(keyword in title for keyword in ["신곡", "반응", "강미나"]):
        return (
            ...
            "10주년 앨범 소식에 먼저 공개된 챌린지까지 붙으면서 팬들 사이에서도 말이 꽤 나왔습니다. "
            "아직 전체 곡을 다 들은 상황은 아니라서, 기사에 나온 장면과 반응만 먼저 정리했습니다. "
    ...
    if all(keyword in title for keyword in ["김연아", "고우림"]) and "강남" in title:
        return (
            ...
            "그래서 이 이슈는 갈등이라기보다 부부들의 생활 토크가 기사 제목으로 커진 흐름에 가까웠습니다. "
            "방송이 공개되면 분위기는 훨씬 가볍게 느껴질 가능성이 있습니다."
        )
    if "한예리" in title and ("백상" in title or "워스트" in title):
        return (
            "한예리의 백상예술대상 드레스 이야기가 다시 올라왔습니다. "
            "워스트 드레서 반응까지 붙었지만, 정작 한예리는 꽤 담담하게 자기 생각을 남겼습니다. "
    ...
    if "손승원" in title and "음주운전" in title:
        return (
            ...
            "이번 글에서는 재판 기사에 나온 쟁점과 다시 언급된 반응을 나눠서 정리해보겠습니다."
        )
    if item.get("safety_flags"):
        return (
            f"{entity} 관련 이야기는 방송 장면과 온라인에서 나온 말이 같이 묶이면서 커졌습니다. "
            "기사 제목은 센 단어가 먼저 보이지만, 실제 내용은 방송에서 어떤 장면이 있었고 그 뒤에 어떤 이야기가 붙었는지에 가깝습니다. "
            "임신설이나 편집 요구설 같은 표현이 들어가면서 반응이 빨리 번졌고, 시청자들이 불편하게 봤던 장면도 다시 언급됐습니다. "
            "이번 글에서는 그 흐름을 한 번에 읽기 쉽게 정리해봤습니다."
        )
    if "김지민" in title and any(keyword in title for keyword in ["시험관", "난임센터"]):
        return (
            "김지민이 시험관 시술 중 겪고 있는 이야기를 방송에서 꺼냈습니다. "
            "난임센터에 사람이 많아 앉을 자리도 없었다는 말이 기사에 담기면서 현실적인 반응도 함께 붙었습니다. "
            "가벼운 예능 토크라기보다 본인이 지나고 있는 과정을 담담하게 꺼낸 장면에 가깝습니다. "
            "그래서 이번 내용은 자극적인 소식보다 담담한 근황 고백에 더 가까웠습니다."
        )
    if "니요" in title and any(keyword in title for keyword in ["동시 연애", "관계"]):
        return (
            ...
            "사생활 이슈인 만큼 기사에 나온 본인의 설명과 이후 반응이 함께 묶인 흐름이었습니다."
        )
    ...
    if first and context:
        return (
            ...
            f"{first} "
            f"{context} "
            "본문에서는 기사에 나온 내용과 반응이 왜 붙었는지 위주로 정리해봤습니다."
        )
    return (
        f"{entity} 관련 이야기가 새로 올라왔습니다. "
        "크게 부풀리기보다는 기사에 나온 사실과 그 주변 반응을 나눠서 담아봤습니다. "
        "본문에서는 핵심 포인트를 몇 개로 나눠 정리해봤습니다."
    )
...
def sensitive_news_summary(item: dict, sentences: list[str]) -> list[str]:
    ...
    if edit and edit != broadcast:
        paragraphs.append(
            ...
            "이 표현이 들어가면서 방송 장면을 다시 찾아보는 반응도 함께 이어졌습니다."
        )
    else:
        paragraphs.append(
            "임신설이나 편집 요구 같은 표현이 붙으면서 이야기가 훨씬 빠르게 퍼졌습니다. "
            "방송 장면과 온라인 반응이 한꺼번에 엮인 흐름으로 보면 됩니다."
        )
    return paragraphs[:3]
    ...
    return [
        (
            ...
            [
                ...
                "사진 한 장만 놓고 보면 스타일 호불호는 당연히 갈릴 수 있습니다. 다만 이번 이야기는 드레스가 예뻤다, 아니었다를 따지는 것보다 당사자가 그 반응을 어떻게 받아들였는지가 더 크게 남았습니다.",
            ],
        ),
        (
            "한예리가 직접 남긴 말",
    ...
    return [
        ...
        (
            ...
            [
                ...
                "기사 흐름상 강미나 언급은 신곡 반응과 팬들의 아쉬움이 함께 묶이면서 나온 대목입니다. 전체 곡이 공개된 뒤에는 지금과 다른 반응도 이어질 수 있습니다.",
            ],
        ),
    ]
    ...
    return [
        (
            ...
            [
                ...
                "이런 내용은 제목으로 보면 굉장히 크게 느껴집니다. 기사에서는 커뮤니티발 주장과 실제 방송 장면이 함께 언급되면서 이슈가 더 커진 흐름이었습니다.",
            ],
        ),
        (
            "방송 장면과 맞물린 말",
            [
                ...
                "방송을 본 시청자들 사이에서도 말이 이어졌습니다. 아무래도 불편하게 본 장면들이 있었고, 그 위에 온라인 주장이 얹히면서 이야기가 더 커진 것으로 보입니다.",
            ],
        ),
        (
            "통편집설로 번진 이유",
            [
                ...
                "통편집이나 편집 요구 같은 표현은 기사 안에서도 온라인 주장과 방송 분량 이야기가 함께 묶이며 등장했습니다. 그래서 본문에서는 기사에 담긴 흐름만 중심으로 정리했습니다.",
            ],
        ),
        (
            "반응이 더 커진 이유",
            [
                "이번 이야기는 방송 장면만 따로 봤을 때보다 온라인에서 붙은 말들이 같이 퍼지면서 더 커졌습니다. 임신설, 편집 요구설, 통편집설 같은 단어가 제목에 들어가니 반응이 빨라질 수밖에 없었습니다.",
                "아무래도 시청자들이 방송에서 불편하게 본 장면들이 있었고, 그 뒤에 온라인 주장에 힘이 붙으면서 반응까지 한꺼번에 몰린 것으로 보입니다.",
            ],
        ),
    ]
    ...
    return [
        ...
        (
            ...
            [
                ...
                "이 부분은 대중 반응이 더 날카로워질 수밖에 없는 대목입니다. 사고 이후의 태도까지 기사에 언급되면서 단순한 실수라는 말로 넘기기 어려운 분위기가 생겼습니다.",
            ],
        ),
        (
            "과거 이력이 함께 언급된 이유",
...
def category_blog_blueprints(item: dict) -> list[tuple[str, list[str], str]]:
    ...
    if has_title_keywords(item, ["위경련", "탈수", "불참"]):
        return [
            ("연달아 비운 일정", ["불참", "축제", "일정"], "팬들이 먼저 걱정한 부분도 여기였습니다. 무대가 아쉬운 건 맞지만, 건강 문제로 여러 일정을 비운 상황이라 회복 이야기가 먼저 남았습니다."),
    ...
    if has_title_keywords(item, ["시험관", "난임센터"]):
        return [
            ...
            ("방송에서 이어질 이야기", ["방송", "사연", "공개"], "프로그램 안에서는 다른 사연과 함께 이어질 예정입니다. 다음 회차에서 어떤 이야기가 더 나올지도 자연스럽게 궁금해지는 부분입니다."),
        ]
    if has_title_keywords(item, ["니요", "동시 연애", "관계"]):
        return [
            ...
            ("아이들과 생활한다는 부분", ["아이", "생활", "함께"], "관계 이야기에 가족 생활이 붙으면서 보는 사람마다 반응이 더 갈릴 수밖에 없습니다. 기사에서는 본인이 설명한 관계 형태가 중심에 놓였습니다."),
            ("계약 무산까지 언급", ["계약", "비즈니스", "무산"], "공개 이후 실제 활동에도 영향이 있었다는 점은 기사에서 따로 볼 만한 부분입니다. 개인 선택이 대중 이미지와 연결되는 지점이기 때문입니다."),
        ]
    return [
        ("처음 나온 이야기", ["공개", "밝", "전했", "올렸", "출연"], f"{lead_entity(item)} 이슈는 첫 문장만 보면 단순한 소식처럼 보입니다. 다만 어떤 장면에서 이 말이 나왔는지까지 붙으면서 흐름이 조금 더 또렷해졌습니다."),
        ("기사에서 잡힌 핵심", ["핵심", "설명", "언급", "발언", "입장"], "제목보다 본문에 실제로 들어간 내용이 더 크게 남았습니다. 자극적인 표현을 덜어내고 기사에 나온 말만 모아보니 이야기가 훨씬 담백해졌습니다."),
        ("사람들이 말한 부분", ["반응", "온라인", "SNS", "논란", "시청자"], "온라인에서는 기사 속 장면이나 발언을 두고 이런저런 말이 이어졌습니다. 글에서는 그중 반복해서 나온 말만 가볍게 정리했습니다."),
        ("조금 더 볼 부분", ["예정", "향후", "앞두", "다음", "계속"], "아직은 한 번에 결론내릴 만한 내용은 많지 않습니다. 지금 나온 흐름을 정리했고, 후속 내용이 나오면 다시 이어서 정리하겠습니다."),
    ]
...
def core_summary_fallback(item: dict) -> str:
    ...
    if item.get("safety_flags"):
        return (
            ...
            "본문에서는 자주 언급된 키워드와 반응을 나눠서 정리했습니다."
        )
    return (
        f"{entity}{particle} 둘러싼 최근 소식입니다. "
        "기사에 나온 발언과 상황만 중심으로 짧게 정리했습니다."
    )
...
def interest_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    return f"기사화된 포인트는 {term_text}입니다. 기사에 나온 내용을 중심으로 가볍게 정리했습니다."
...
def public_reaction_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    if item.get("safety_flags"):
        return (
            ...
            "처음에는 장면에 대한 불편함이었고, 이후에는 편집 이야기까지 붙으면서 말이 더 많아진 분위기입니다."
        )
    return (
        f"{entity} 관련 반응을 넓게 정리할 만한 내용은 아직 많지 않습니다. "
        "현재 기사에는 언급된 발언과 상황이 짧게 담겼습니다."
    )
...
def extra_context_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    if item.get("safety_flags"):
        return (
            f"{entity} 관련 이야기는 여러 표현이 한꺼번에 묶여 나온 흐름입니다. "
...
def closing_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    if "손승원" in title and "음주운전" in title:
        return (
            ...
            "반복된 문제라는 점에서 더 무겁게 느껴졌고, 이번 일을 계기로 책임 있는 태도와 분명한 변화가 뒤따라야 한다고 생각합니다."
        )
    if item.get("safety_flags"):
        return (
            f"{entity} 관련 이야기는 방송 장면과 온라인 반응이 한꺼번에 엮이면서 생각보다 크게 번졌습니다. "
            "개인적으로는 이후 방송에서 이 흐름이 어떻게 정리될지도 궁금해지는 이야기였습니다."
        )
    if "한예리" in title and ("백상" in title or "워스트" in title):
        return (
    ...
    if related_articles:
        return (
            ...
            "지금 나온 내용을 차분히 정리했고, 좋은 방향의 후속 소식도 함께 기다리게 됩니다."
        )
    return (
        f"{entity} 이야기는 제목보다 기사 안에 나온 말이 먼저 남습니다. "
        "짧은 소식이어도 그 안에서 남는 포인트가 있었고, 다음 흐름도 자연스럽게 이어질 것 같습니다."
    )
