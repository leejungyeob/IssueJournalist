...
def blog_summary(item: dict, related_articles: list[dict]) -> str:
    ...
    if item.get("safety_flags"):
        return (
            ...
            "제목에 센 표현이 들어가다 보니 더 크게 보이지만, 확인된 내용과 추측처럼 보이는 부분은 따로 나눠서 정리하는 게 자연스러웠습니다. "
    ...
    if first and context:
        return (
            ...
            f"{first} "
            f"{context} "
            "본문에서는 확인된 내용과 반응이 왜 붙었는지 위주로 정리해봤습니다."
        )
    return (
...
def category_blog_blueprints(item: dict) -> list[tuple[str, list[str], str]]:
    ...
    return [
        ...
        ("기사에서 잡힌 핵심", ["핵심", "설명", "언급", "발언", "입장"], "제목보다 본문에 실제로 들어간 내용이 더 크게 남았습니다. 자극적인 표현을 덜어내고 확인된 말만 모아보니 이야기가 훨씬 담백해졌습니다."),
        ("반응이 붙은 이유", ["반응", "온라인", "SNS", "논란", "시청자"], "반응은 늘 기사 내용보다 빠르게 커집니다. 그래도 어떤 부분이 사람들에게 걸렸는지 보면 이 이슈가 왜 이어졌는지는 어느 정도 보입니다."),
        ("조금 더 볼 부분", ["예정", "향후", "앞두", "다음", "계속"], "아직은 한 번에 결론내릴 만한 내용은 많지 않습니다. 지금 확인된 흐름만 정리했고, 후속 내용이 나오면 다시 이어서 정리하겠습니다."),
    ]
...
def interest_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    if item.get("safety_flags"):
        return (
            ...
            "기사 안에서는 확인된 장면과 온라인 주장성 표현이 함께 다뤄졌습니다."
        )
