...
def blog_summary(item: dict, related_articles: list[dict]) -> str:
    ...
    if item.get("safety_flags"):
        return (
            ...
            "특히 임신설이나 편집 요구설처럼 민감한 말은 기사에 나온 표현 그대로 받아들이기엔 조금 조심스럽게 느껴졌습니다. "
...
def sensitive_news_summary(item: dict, sentences: list[str]) -> list[str]:
    ...
    if edit and edit != broadcast:
        paragraphs.append(
            ...
            "다만 이 부분은 아직 주장성 내용이라 사실처럼 단정하긴 조심스럽습니다."
        )
    else:
        paragraphs.append(
            "다만 임신설이나 편집 요구처럼 민감한 표현은 아직 확인된 사실처럼 받아들이기엔 조심스럽습니다. "
    ...
    return [
        (
            ...
            [
                ...
                "이런 내용은 제목으로 보면 굉장히 크게 느껴집니다. 다만 커뮤니티발 주장과 실제 방송 내용을 같은 무게로 놓기엔 조금 조심스러운 부분이 있었습니다.",
            ],
        ),
        (
            "방송 장면과 맞물린 말",
