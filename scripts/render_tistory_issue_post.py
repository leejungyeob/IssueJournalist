    ...
    return [
        ...
        (
            ...
            [
                ...
                "아이오아이 이름으로 나오는 10주년 소식이다 보니 멤버별 언급도 자연스럽게 나왔습니다. 강미나 이름이 함께 나온 것도 그런 기대와 아쉬움이 겹친 쪽에 가까워 보입니다.",
            ],
        ),
    ]
    ...
    return [
        ...
        (
            ...
            [
                ...
                "이후 방송 분량을 두고도 댓글에서 여러 말이 나왔습니다. 익명 글에 적힌 요청 과정이 통편집설과 연결되며 반응이 커진 모습입니다.",
            ],
        ),
    ]
...
def category_blog_blueprints(item: dict) -> list[tuple[str, list[str], str]]:
    ...
    return [
        ...
        ("이어질 일정과 배경", ["예정", "향후", "앞두", "다음", "계속"], "기사에는 이후 일정이나 최근 활동도 함께 언급됐습니다. 그 배경까지 들어가면서 오늘 나온 소식이 어디에 이어지는지도 보입니다."),
    ]
...
def closing_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    if item.get("safety_flags"):
        return (
            ...
            "방송에서 나온 장면과 익명 글이 함께 나오면서 관련 기사도 빠르게 이어졌습니다."
        )
    if "한예리" in title and ("백상" in title or "워스트" in title):
        return (
