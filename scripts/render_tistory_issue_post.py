...
def blog_summary(item: dict, related_articles: list[dict]) -> str:
    ...
    if "아이오아이" in title and any(keyword in title for keyword in ["신곡", "반응", "강미나"]):
        return (
            ...
            "아직 전체 곡을 다 들은 상황은 아니라서, 몇 초짜리 첫 구간이 먼저 기사 제목에 잡힌 셈입니다. "
            "팬들이 기다린 10주년 앨범이라는 점까지 더해져 짧은 영상에도 말이 꽤 붙었습니다."
        )
    ...
    if item.get("safety_flags"):
        return (
            ...
            "기사 제목에는 임신설과 통편집설이 먼저 보이지만, 기사에는 방송에서 어떤 장면이 있었고 그 뒤에 어떤 이야기가 붙었는지가 담겨 있습니다. "
            "임신설이나 편집 요구설 같은 표현이 들어가면서 반응이 빨리 번졌고, 시청자들이 불편하게 봤던 장면도 다시 언급됐습니다. "
            "방송 장면과 익명 글이 함께 묶이면서 이야기가 더 커진 경우였습니다."
        )
...
def sensitive_news_summary(item: dict, sentences: list[str]) -> list[str]:
    ...
    if edit and edit != broadcast:
        ...
        paragraphs.append(
            ...
            "방송에서 나온 장면과 온라인 글이 같은 기사 안에서 함께 다뤄졌습니다."
        )
    return paragraphs[:3]
    ...
    return [
        (
            ...
            [
                ...
                "드레스는 상체 쪽 꽃 장식이 크게 들어간 디자인이라 사진만 봐도 시선이 먼저 갔습니다. 그래서 레드카펫 의상 자체보다 그 장식을 두고 나온 말들이 기사에 함께 붙었습니다.",
            ],
        ),
        (
            "한예리가 직접 남긴 말",
        ...
        (
            "달걀프라이 반응까지 나온 장식",
            [
                "온라인에서는 가슴 부분의 꽃 장식을 두고 '달걀프라이 같다'는 식의 반응도 나왔습니다. 기사 제목에 이 표현이 함께 들어가면서 드레스 이야기가 더 크게 보였습니다.",
        ...
        (
            ...
            [
                ...
                "레드카펫 의상은 늘 호불호가 갈리지만, 이번 기사에서는 드레스 평가만큼이나 한예리가 직접 남긴 말이 같이 읽히는 편이었습니다.",
            ],
        ),
    ]
    ...
    return [
        ...
        (
            ...
            [
                ...
                "미니앨범 제목과 발매 소식이 먼저 알려진 뒤 챌린지 영상이 붙었습니다. 팀을 오래 기다린 팬들 입장에서는 신곡 한 곡보다 10주년이라는 시간이 같이 보이는 소식이었습니다.",
            ],
        ),
        (
            "엇갈린 신곡 반응",
        ...
        (
            ...
            [
                ...
                "아이오아이 이름으로 나오는 10주년 소식이다 보니 멤버별 언급도 자연스럽게 이어졌습니다. 강미나 이름이 함께 나온 것도 그런 기대와 아쉬움이 같이 붙은 쪽에 가까워 보입니다.",
            ],
        ),
    ]
    ...
    return [
        (
            ...
            [
                ...
                "기사에는 익명 커뮤니티발 주장과 방송 장면이 함께 등장했습니다. 그래서 제목에는 임신설, 편집 요구, 통편집 같은 단어가 한꺼번에 잡혔습니다.",
            ],
        ),
        (
            "방송 장면과 맞물린 말",
        ...
        (
            ...
            [
                ...
                "통편집이나 편집 요구 같은 표현은 방송 분량 이야기와 함께 언급됐습니다. 실제 방송에서 분량이 줄어든 듯 보였다는 말까지 붙으면서 이 부분이 따로 제목에 잡혔습니다.",
            ],
        ),
        (
            "시청자 반응까지 이어진 장면",
            [
                "방송에서는 순자가 눈물을 보인 장면이 먼저 언급됐습니다. 그 장면을 본 뒤 옥순의 말과 태도를 불편하게 봤다는 반응도 같이 이어졌습니다.",
                "이후 익명 글에서 나온 주장까지 더해지면서 방송 비판과 사생활 추측이 한 기사 안에 같이 들어온 상황입니다.",
            ],
        ),
    ]
    ...
    return [
        ...
        (
            ...
            [
                ...
                "과거 실형과 군 면제 이야기까지 다시 소환되면서 기사 무게가 더 커졌습니다. 이번 재판 소식은 새 사건만 따로 나온 게 아니라 이전 이력과 함께 다뤄졌습니다.",
            ],
        ),
    ]
...
def category_blog_blueprints(item: dict) -> list[tuple[str, list[str], str]]:
    ...
    if has_title_keywords(item, ["위경련", "탈수", "불참"]):
        return [
            ...
            ("컴백을 앞둔 프로미스나인", ["컴백", "앨범", "활동", "예정"], "프로미스나인은 7월 정규 앨범 컴백을 앞두고 있습니다. 컴백 준비와 행사 일정이 겹치는 시기라 컨디션 관리 이야기도 자연스럽게 따라붙었습니다."),
        ]
    if has_title_keywords(item, ["시험관", "난임센터"]):
        return [
            ...
            ("방송에서 이어질 이야기", ["방송", "사연", "공개"], "프로그램 안에서는 다른 사연과 함께 이어질 예정입니다. 김지민의 이야기도 그 안에서 한 번 더 다뤄질 것으로 보입니다."),
        ]
    if has_title_keywords(item, ["니요", "동시 연애", "관계"]):
        return [
            ...
            ("아이들과 생활한다는 부분", ["아이", "생활", "함께"], "기사에서는 아이들과 함께 생활한다는 내용도 따로 언급됐습니다. 관계 설명에 가족 생활이 함께 붙으면서 소식의 결이 더 복잡해졌습니다."),
            ("계약 무산까지 언급", ["계약", "비즈니스", "무산"], "공개 이후 실제 활동에도 영향이 있었다는 점도 기사에 함께 들어갔습니다. 개인적인 선택이 대중 이미지와 연결된 장면이었습니다."),
        ]
    return [
        ...
        ("이어질 일정과 배경", ["예정", "향후", "앞두", "다음", "계속"], "기사에는 이후 일정이나 최근 활동도 함께 언급됐습니다. 그 배경까지 붙으면서 오늘 나온 소식이 어디에 이어지는지도 보입니다."),
    ]
...
def closing_paragraph(item: dict, related_articles: list[dict]) -> str:
    ...
    if "손승원" in title and "음주운전" in title:
        return (
            ...
            "반복된 문제라는 점에서 더 무겁게 느껴졌고, 재판 과정에서 나온 말들도 가볍게 지나가기 어려웠습니다."
        )
    if item.get("safety_flags"):
        return (
            f"{entity} 관련 이야기는 방송 장면과 온라인 반응이 한꺼번에 엮이면서 생각보다 크게 번졌습니다. "
            "방송에서 나온 장면과 익명 글이 같이 묶이면서 관련 기사도 빠르게 이어졌습니다."
        )
    if "한예리" in title and ("백상" in title or "워스트" in title):
        return (
    ...
    if related_articles:
        return (
            ...
            "지금 나온 내용 안에서는 기사 속 발언과 최근 일정이 함께 남았습니다."
        )
    return (
        f"{entity} 이야기는 제목보다 기사 안에 나온 말이 먼저 남습니다. "
        "짧은 소식이어도 그 안에서 나온 발언과 장면이 이어져 있었습니다."
    )
